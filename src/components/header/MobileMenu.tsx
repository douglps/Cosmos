...
export function MobileMenu({ isOpen, closeMenu, menuRef }: MobileMenuProps) {
  ...
  return (
    <div
      ref={menuRef}
      className={`transition-all duration-300 ease-in-out transform lg:hidden absolute top-20 right-2 z-50 bg-[#ffe9d4] border border-black dark:bg-slate-700 text-black dark:text-white rounded-sm shadow-2xl p-4 w-40 ${
        isOpen
    ...
    >
      <div className="flex justify-center ml-4 mr-4 items-center mb-4">
        <div
          onClick={closeMenu}
          className="focus:outline-none absolute top-0 right-0 bg-[#ffe9d4] dark:bg-lime-950 rounded-md"
        >
